...
function createtime1() {
  var grt = new Date("08/04/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 汤月雯🥝 %c 你正在访问汤月雯🥝の博客",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 汤月雯🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('汤月雯🥝的博客内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (m == 1 && dd == 27) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝汤月雯" + (y - 2001).toString() + "岁生日快乐！🥝");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
// if (m == 6 && dd == 30) {//小猫咪生日
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("08/04/2024 00:00:00");
